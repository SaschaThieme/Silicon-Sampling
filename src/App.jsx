...
const STEPS = ["Zielgruppe", "Fragebogen", "Simulation", "Ergebnisse"];
...
const QUESTION_TEMPLATES = [
  "Wie zufrieden sind Sie generell mit [Thema]?",
  "Was ist Ihr größtes Problem bei [Thema]?",
  "Würden Sie [Thema] weiterempfehlen? Warum?",
  "Welchen Preis empfinden Sie für [Thema] als fair?",
  "Was müsste sich ändern, damit Sie [Thema] kaufen würden?",
];

// ── Eurostat Bevölkerungsabfrage ──────────────────────────────────────────────
// Quelle: Eurostat demo_pjan — Bevölkerung am 1. Januar nach Einzeljahr, Deutschland

async function fetchBevoelkerungDE(alterStr) {
  const match = alterStr?.match(/(\d+)[–\-]+(\d+)/);
  if (!match) return null;
  const from = parseInt(match[1]);
  const to   = parseInt(match[2]);
  const ages = Array.from({ length: to - from + 1 }, (_, i) => `Y${from + i}`).join('|');
  const url = `https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/demo_pjan?format=JSON&geo=DE&sex=T&time=2023&age=${ages}`;
  const resp = await fetch(url);
  if (!resp.ok) throw new Error(`Eurostat ${resp.status}`);
  const json = await resp.json();
  return Object.values(json.value || {}).reduce((s, v) => s + (v || 0), 0);
}
...
function BevoelkerungBadge({ alter, fallback, style: extraStyle, plain }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    fetchBevoelkerungDE(alter)
      .then(n => { if (!cancelled) setVal(n); })
      .catch(() => { if (!cancelled) setVal(null); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [alter]);

  const numText = loading
    ? '…'
    : val != null
  ...
  // plain mode: just the number, styled like other tile values
  if (plain) {
  ...
  return (
    <div style={{ display: "inline-block", fontSize: 10, fontWeight: 700, padding: "2px 8px",
      ...
      🇩🇪 {numText} in DE {val != null && <span style={{ fontWeight: 400, opacity: 0.7 }}>· Eurostat 2023</span>}
    </div>
  );
}
...
export default function SiliconSamplingApp() {
  ...
  const canProceed = () => {
    if (step === 0) {
      if (personaType === "b2b") return persona.label && persona.branche && topic;
      return persona.label && persona.alter && topic;
    }
    if (step === 1) return questions.filter(q => q.trim()).length > 0;
    return true;
  };
  ...
  const runSimulation = async () => {
    ...
    const sysPrompt = personaType === "b2b"
...
Thema: ${topic}
Antworte professionell und präzise (2-4 Sätze/Frage), mit betriebswirtschaftlicher Perspektive, auf Deutsch. Variiere leicht.
...
Thema: ${topic}
Antworte authentisch, kurz (2-4 Sätze/Frage), mit emotionaler Nuance, auf Deutsch. Variiere leicht.
NUR JSON: {"answers":[{"question":"...","answer":"..."}],"sentiment":"positiv|neutral|negativ","nps":<0-10>}`;
  ...
  return (
    <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", background: C.bgSoft, minHeight: "100vh", color: C.text }}>
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "40px 32px 60px" }}>
        ...
        {step === 0 && (
          <div>
            <StepHeader num={1} title="Zielgruppe & Thema" subtitle="Wähle eine gespeicherte Persona oder erstelle eine neue." accent={C.blue} />
                    ...
                    {(personaType === "b2c" ? PERSONA_PRESETS_B2C : PERSONA_PRESETS_B2B)
                      ...
                      return (
                        <div key={i} style={{ position: "relative", padding: "14px 18px", borderRadius: 10, cursor: "pointer", border: `2px solid ${C.border}`, background: C.bg, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", transition: "all 0.15s" }}
                          onClick={() => { setPersona({ ...(p.alter || p.branche ? p : { label: p.label }) }); setPersonaMode("create"); }}>
                          {/* (x) delete button */}
              ...
              <div>
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px 28px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", marginBottom: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px 20px", marginBottom: 20 }}>
                    ...
                    <Field label="Bildung" value={persona.bildung} onChange={v => setPersona({ ...persona, bildung: v })} placeholder="z.B. Hochschulabschluss" />
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "16px 20px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                  <div style={{ fontSize: 13, color: C.textMid }}>💾 Persona für spätere Simulationen speichern</div>
                  {!showSaveInput ? (
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px 28px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
                  ...
                  <input value={topic} onChange={e => setTopic(e.target.value)}
                    placeholder="Worum geht es in dieser Studie? z.B. Einführung einer neuen digitalen Plattform für mittelständische Unternehmen"
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px 28px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
                  <Label>Forschungsthema *</Label>
                  <input value={topic} onChange={e => setTopic(e.target.value)}
                    placeholder="Worum geht es in dieser B2B-Studie? z.B. Einführung einer neuen SaaS-Lösung für den Mittelstand"
                    style={inputStyle(C.blue)} />
                </div>
                </div>)} {/* end B2B */}
              </div>
        ...
        {step === 1 && (
          <div>
            <StepHeader num={2} title="Fragebogen erstellen" subtitle="Formuliere die Fragen für deine synthetischen Respondenten." accent="#7B3FA0" />
        ...
        {step === 2 && (
          <div>
            <StepHeader num={3} title="Simulation starten" subtitle="Die KI befragt jetzt die synthetischen Respondenten deiner Zielgruppe." accent={C.pink} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${persona.alter ? 5 : 4},1fr)`, gap: 14, marginBottom: 28 }}>
              {[
                ...
                { k: "Region", v: (persona.region || "–").split("(")[0].trim(), s: persona.einkommen || "–" },
              ...
              {persona.alter && (
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                  ...
                  <div style={{ fontSize: 11, color: C.textMid, marginTop: 2 }}>in Deutschland · Eurostat 2023</div>
                </div>
              )}
            </div>
            ...
            {!simulating && results.length === 0 && !error && (
              <div style={{ textAlign: "center", padding: "40px 0" }}>
                <button onClick={() => { runSimulation(); setStep(3); }} style={{
        ...
        {step === 3 && (
          <div>
            {simulating && (
              <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px 28px", marginBottom: 28, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>Befrage synthetische Respondenten…</div>
                  <div style={{ fontSize: 13, color: C.blue, fontWeight: 700 }}>{results.length} / {sampleSize}</div>
                </div>
                <div style={{ height: 6, background: C.borderLight, borderRadius: 3, overflow: "hidden", marginBottom: 16 }}>
                  <div style={{ height: "100%", width: `${progress}%`, background: `linear-gradient(90deg, ${C.blue}, ${C.pink})`, borderRadius: 3, transition: "width 0.5s ease" }} />
                </div>
                {results.slice(-3).map((r, i) => (
                  <div key={i} style={{ padding: "10px 16px", background: C.bgSoft, borderRadius: 8, border: `1px solid ${C.border}`, display: "flex", gap: 12, alignItems: "center", fontSize: 13, marginBottom: 6 }}>
                    <span style={{ color: C.textLight, minWidth: 72 }}>Person {r.id}</span>
                    ...
                    <span style={{ color: C.textMid }}>NPS {r.nps}/10</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {results.length > 0 && (
              <>
                ...
                <div style={{ marginTop: 28, display: "flex", gap: 10 }}>
                  <button onClick={() => { setStep(0); setResults([]); setSummary(""); setProgress(0); setPersona(personaType === "b2b" ? { ...EMPTY_PERSONA_B2B } : { ...EMPTY_PERSONA_B2C }); setTopic(""); setQuestions([""]); setPersonaMode("select"); }}
                  ...
                  <button onClick={() => { setStep(0); setResults([]); setSummary(""); setProgress(0); }}
        ...
        {step === 2 && results.length === 0 && !simulating && (
